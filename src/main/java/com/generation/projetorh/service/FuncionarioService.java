...
public class FuncionarioService {
    ...
    public Optional<Funcionario> cadastrarFuncionario(Funcionario funcionario) {
        ...
        boolean departamentoExiste = departamentoRepository
                .existsById(funcionario.getDepartamento().getId());

        if (!departamentoExiste) {
            return Optional.empty();
        }

        BigDecimal salario = funcionario.getValorHora()
                .multiply(BigDecimal.valueOf(funcionario.getHorasTrabalhadas()))
                .subtract(funcionario.getDescontos());
